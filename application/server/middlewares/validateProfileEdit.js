...
const validateProfileEdit = (req, res, next) => {
    ...
    if (firstName && (typeof firstName !== 'string' || firstName.trim().length < 2)) {
        return res.json({ success: false, message: 'Invalid first name. It must be at least 2 characters long.' });
    }

    // Validation for last name
    if (lastName && (typeof lastName !== 'string' || lastName.trim().length < 2)) {
        return res.json({ success: false, message: 'Invalid last name. It must be at least 2 characters long.' });
    }

    // Validation for 'major' field
    if (major && typeof major !== 'string') {
        return res.json({ success: false, message: 'Invalid major. It must be a string.' });
    }

    // Validation for 'minor' field
    if (minor && typeof minor !== 'string') {
        return res.json({ success: false, message: 'Invalid minor. It must be a string.' });
    }

    // Validation for 'pronouns' field
    if (pronouns && typeof pronouns !== 'string') {
        return res.json({ success: false, message: 'Invalid pronouns. It must be a string.' });
    }
