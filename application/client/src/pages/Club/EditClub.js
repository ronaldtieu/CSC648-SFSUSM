...
const EditClub = ({ token }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        // Call the update endpoint with the new details
        try {
            const response = await updateClub({ groupId: id, groupName: clubName, description }, token);
            if (response.success) {
                // Redirect back to the club page with the updated data using useHistory
